...
export function indexComparator(
  { index: itemIndex }: OffsetPoint,
  index: number,
) {
  return index === itemIndex ? 0 : index < itemIndex ? -1 : 1;
}

export function offsetComparator(
  { offset: itemOffset }: OffsetPoint,
  offset: number,
) {
...
export function rangesWithinOffsets(
  ...
  if (minStartIndex > 0) {
    startOffset = Math.max(
      startOffset,
      arrayBinarySearch.findClosestSmallerOrEqual(
        tree,
        minStartIndex,
        indexComparator,
      ).offset,
  ...
  return arrayToRanges(
    arrayBinarySearch.findRange(tree, startOffset, endOffset, offsetComparator),
    offsetPointParser,
  );
}
...
export function offsetOf(index: number, tree: Array<OffsetPoint>) {
  ...
  const {
    ...
    index: startIndex,
    size,
  } = arrayBinarySearch.findClosestSmallerOrEqual(tree, index, indexComparator);
  return size * (index - startIndex) + offset;
}

export function originalIndexFromItemIndex(
  itemIndex: number,
  sizes: SizeState,
) {
  if (!hasGroups(sizes)) {
        ...
        u.map(([groupIndices, sizes]) => {
          const groupOffsetTree = groupIndices.reduce((tree, index, idx) => {
            return insert(
              tree,
              index,
              offsetOf(index, sizes.offsetTree) || idx,
            );
          }, newTree<number>());
